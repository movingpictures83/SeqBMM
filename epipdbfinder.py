...
from utils import obtain_lock, release_lock, wait_unlock
...
def hit_to_pdb(
    ...
    for row in mmseqs_db_rows:
        ...
        if pdbhit.isAF and pdbhit.gplddt < args.gplddt:
            continue
        hit_to_csv(
            f"EPI_PDB_hits_{pdb_input_str}.tsv",
            motif,
            query_pdb_res_nums,
            motif_acc,
            pdbhit,
        ...
        if idx != -1:
            ...
            query_pdb_filename = f"{query_structure_basename}_{query_pdb_chain}_{query_pdb_res_nums[0]}-{query_pdb_res_nums[-1]}.pdb"
            ...
            if not os.path.exists(query_pdb_filename) and obtain_lock(query_pdb_filename):
                try:
                    extract(
                        f"{PDB_DATABASE_DIR}/{query_structure_basename[1:3]}/{query_structure_basename}.cif",
                        query_pdb_chain,
                    ...
                    release_lock(query_pdb_filename)
                    #print(f"[bold red] {query_structure_basename}.cif not Found")
                    continue
            else:
            ...
            #try:
            target_pdb_seqs = MMCIFSeqs(target_base_pdb_name, target_chain_pdb_name, SQLITE_DATABASE_DIR, compute_acc=True)
            #except:
            ...
            target_pdb_filename = f"{target_base_pdb_name}_{target_chain_pdb_name}_{pdbhit.motif_res_nums_target[0]}-{pdbhit.motif_res_nums_target[-1]}.pdb"
            if not os.path.exists(target_pdb_filename) and obtain_lock(target_pdb_filename):
                try:
                    extract(
                        f"{AFDB_DIR}/{target_base_pdb_name}.cif"
                        if pdbhit.isAF
                        else f"{PDB_DATABASE_DIR}/{target_base_pdb_name[1:3]}/{target_base_pdb_name}.cif",
                    ...
                    release_lock(target_pdb_filename)
                    #print(f"[bold red] {target_base_pdb_name}.cif not Found")
                    continue
            else:
            ...
            if not os.path.exists(f"{TMalign_prefix}.stdout") and obtain_lock(
                f"{TMalign_prefix}.stdout"
            ):

                # force TM to properly align using  motif sequence alignment
                with open(f"{TMalign_prefix}.fa", "w") as tmfasta:
                    tmfasta.write(
                ...
                if result.stderr:
                    release_lock(f"{TMalign_prefix}.stdout")
                    # print(result.stderr)
                    continue
                with open(f"{TMalign_prefix}.stdout", "w") as output_handle:
                    output_handle.write(result.stdout)
                release_lock(f"{TMalign_prefix}.stdout")
            else:
                wait_unlock(f"{TMalign_prefix}.stdout")

            # read in tmalign output

            with open(f"{TMalign_prefix}.stdout") as input_handle:
            ...
            pdbhit.TMalign_PDB_file = f"{TMalign_prefix}_atm.pdb"

            #if args.rmsd:
            #    if float(pdbhit.TMalign_RMSD) >= args.rmsd:
            #        continue
            hit_to_csv(
                f"EPI_PDB_fragment_pairs_{pdb_input_str}.tsv",
                motif,
                query_pdb_res_nums,
                motif_acc,
                pdbhit,
